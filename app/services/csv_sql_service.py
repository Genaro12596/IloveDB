...
def _infer_column_type(column_name: str, values: list) -> str:
    ...
    for value in cleaned_values:
        ...
        if is_int and not (value.isdigit() or (value.startswith('-') and value[1:].isdigit())):
            is_int = False
        ...
        if is_bool and value.lower() not in ('true', 'false', '1', '0', 'verdadero', 'falso'):
            is_bool = False
        # Comprobar si es fecha (YYYY-MM-DD o YYYY-MM-DD HH:MM:SS)
        if is_date and not re.match(r'^\d{4}-\d{2}-\d{2}( \d{2}:\d{2}:\d{2})?$', value):
            is_date = False

    if is_bool: return 'BOOLEAN'
    if is_int: return 'INT'
    if is_float: return 'DECIMAL(10, 2)'
    if is_date: return 'TIMESTAMP' if ':' in cleaned_values[0] else 'DATE'
...
def process_csv_to_sql(table_name: str, csv_data: str) -> dict:
    ...
    if len(rows) == 1 or not _looks_like_header(rows[0]):
        headers = [f'col_{column_index+1}' for column_index in range(len(rows[0]))]
        data_rows = rows
    else:
        headers = [header.strip().replace(" ", "_").lower() for header in rows[0]]
        data_rows = rows[1:]
    ...
    column_definitions = [f"    {header} {column_type}" for header, column_type in zip(headers, column_types)]
    ...
    for row in data_rows:
        ...
        for column_index, value in enumerate(row):
            value = value.strip()
            col_type = column_types[column_index] if column_index < len(column_types) else 'VARCHAR(255)'
